...
data <- rbind(data, data.frame(date="2020-08-10", state="Hawaii", fips=NA, cases=data$cases[nrow(data)] + 153, deaths=NA))
# ----------------------------------------------------------- #
...
for (i in 35:0) {
  ...
  plot(cases ~ day, phase1, main = "Hawai'i: logistic growth models", xlab = "Days since March 7", ylab = "Positive cases", xlim = c(1, 250), ylim = c(0, 7500), pch=1, col=rgb(0,0,0,0.3), type="p")  # Census data
