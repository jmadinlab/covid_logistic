...
data <- rbind(data, data.frame(date="2020-07-31", state="Hawaii", fips=NA, cases=data$cases[nrow(data)] + 123, deaths=NA))
# ----------------------------------------------------------- #

data$day <- 1:nrow(data)
lock <- "2020-04-30" # lockdown lifted
phase1 <- data[1:which(data$date==lock),]
phase1_mod <- nlsLM(cases ~ SSlogis(day, phi1, phi2, phi3), phase1)
summary(phase1_mod)
...
for (i in 0:35) {
  ...
  phase2_mod <- nlsLM(cases_adj ~ SSlogis(day_adj, phi1, phi2, phi3), phase2)
  summary(phase2_mod)
